...
def load_judgments_df_from_output_file():
    # Load new datasets into data frame
    df_judgments = pd.DataFrame()
    print('Loading judgments from output file ...')
...
def generate_pairs(uses, judgments_df_filtered):
    ...
    judgments_df_filtered_new = judgments_df_filtered[judgments_df_filtered['lemma'] == lemma]

    # For each judgment, look up the corresponding uses by its identifiers
    for index, judgment in judgments_df_filtered_new.iterrows():
        # print('Processing judgment', judgment)

        judgment_mean = str(judgment[4:13].mean())
...
def generate_and_write_pairs_challenge():
    ...
    judgments_aggregated = aggregate_judgments_df(load_judgments_df_from_output_file())
