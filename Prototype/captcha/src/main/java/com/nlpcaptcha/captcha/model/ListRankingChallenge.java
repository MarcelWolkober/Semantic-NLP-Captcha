...
public class ListRankingChallenge implements Serializable {
    ...
    public String[] convertIdOrderToIdentifiers(JSONArray userOrder){
        String[] identifiers = new String[userOrder.length()];
        for (int i = 0; i < userOrder.length(); i++) {
            int finalI = i;
            identifiers[i] = this.getListUsages().stream().filter(usage -> usage.getId().equals(userOrder.getLong(finalI))).findFirst().get().getIdentifier();
        }
        return identifiers;
    }
